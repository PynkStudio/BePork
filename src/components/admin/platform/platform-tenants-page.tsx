...
export type PlatformTenantsPageProps = {
  /**
   * Blocca la vista sui tenant del verticale indicato (portale prodotto):
   * il filtro cross-prodotto sparisce e gli altri tenant non sono raggiungibili.
   * Omesso = tutti i verticali (hub PynkStudio e admin.menuary.it).
   */
...
export function PlatformTenantsPage({
  ...
  // In un portale prodotto i tenant degli altri verticali non devono essere
  // raggiungibili neanche via modale, quindi si restringe il pool alla radice.
  const scopedTenants = useMemo(
    () => (vertical ? ADMIN_TENANTS.filter((tenant) => tenant.vertical === vertical) : ADMIN_TENANTS),
    [vertical],
  );

  const filteredTenants = useMemo(
    () =>
      scopedTenants.filter((tenant) => verticalFilter === "all" || tenant.vertical === verticalFilter),
    [scopedTenants, verticalFilter],
  );

  const advancedTenant = advancedOpenFor
    ? scopedTenants.find((tenant) => tenant.id === advancedOpenFor)
    : undefined;
        ...
        <div className="flex flex-col gap-4 border-b border-pork-ink/10 pb-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="inline-flex items-center gap-2 text-sm font-black text-pork-ink">
              ...
              {vertical ? "Tenant del prodotto" : "Filtro vertical"}
            </div>
            <p className="mt-1 text-xs text-pork-ink/50">
              {filteredTenants.length} tenant visibili su {scopedTenants.length}.
            </p>
          </div>
          {!vertical && (
              ...
              <section className="rounded-2xl bg-pork-cream p-4">
                <div className="grid gap-3 md:grid-cols-3">
                  <div>
                    ...
                    <Link href={`${crmBasePath}/${lead.id}`} className="mt-1 inline-flex items-center gap-1 text-sm font-black text-pork-red hover:underline">
