...
async function fetchOperationalLocations(tenantSlug: string): Promise<LocationChoice[]> {
  ...
  return (data ?? []) as LocationChoice[];
}

async function resolveLocationId(tenantSlug: string, locationSlug?: string): Promise<string | null> {
...
export default async function OrdiniPage({
  ...
  const locations = !auth.isDemo && tenant.features.multiLocation ? await fetchOperationalLocations(tenantSlug) : [];
  if (locations.length > 1 && !locationSlug) {
    return (
      <div className="ga-dashboard">
        <header>
          <span className="ga-eyebrow">Operatività</span>
          <h1 className="ga-heading">Scegli sede</h1>
          <p className="ga-lead">Ogni sede ha la propria console ordini, con coda e stati separati.</p>
        </header>
        <div className="ga-kpi-grid">
          {locations.map((l) => (
            <Link key={l.id} href={`?loc=${l.slug}`} className="ga-card" style={{ textDecoration: "none" }}>
              <h2 className="ga-section-title">{l.name}</h2>
              <p className="ga-section-hint" style={{ marginTop: 6 }}>
                Apri ordini sede {l.slug}
              </p>
            </Link>
          ))}
        </div>
