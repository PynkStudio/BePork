 ...
 *   "gestione.bepork"        → admin.pynkstudio.eu/bepork    (pannello store food)
...
export function resolveDestination(options: {
  ...
  if (from === "studio") {
    ...
    if (tenantId)    return `https://admin.pynkstudio.eu/${tenantId}/fatturazione${next ?? ""}`;
  }
  ...
  if (from?.startsWith("gestione.")) {
    ...
    if (slug && (isSiteadmin || tenantId === slug))
      return `https://admin.pynkstudio.eu/${slug}${next ?? ""}`;
  }
  ...
  // Fallback per ruolo
  if (isSiteadmin) return "https://admin.menuary.it/admin";
  if (tenantId)    return `https://admin.pynkstudio.eu/${tenantId}`;
